...
/**
 * 双滑块拖动条
 * 原作者出处：https://juejin.cn/post/6844903810201042951
 */
public class DoubleSeekBar extends View {
    ...
    public DoubleSeekBar(Context conteFxt) {
        this(context, null);
    }
    ...
    public DoubleSeekBar(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        bgHeight = dp2px(context, 6);//设置背景线条的高度
        ...
        paintBg.setColor(ContextCompat.getColor(context, R.color.cyan));
        paintBg2.setColor(ContextCompat.getColor(context, R.color.gray));
    ...
    private boolean handleMoveEvent(MotionEvent motionEvent) {
        ...
        if (clickedType == CLICK_TYPE_LEFT) {
            if (x < indexRightX - leftIcon.getWidth() && x > startLeftX) {//左按钮的范围小于右按钮的位置，大于初始值位置
                indexLeftX = x;
            } else if (x >= indexRightX -  leftIcon.getWidth()) {
                indexLeftX = indexRightX - leftIcon.getWidth();
            } else if (x <= startLeftX) {
            ...
            if (x > indexLeftX + rightIcon.getWidth() && x < startRightX) {//右按钮的范围大于左按钮的位置，小于初始值位置
                indexRightX = x;
            } else if (x <= indexLeftX + rightIcon.getWidth()) {
                indexRightX = indexLeftX + rightIcon.getWidth();
            } else if (x >= startRightX) {
